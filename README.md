...
PyVISAパッケージは，実験に使用する測定機器との通信を行う際の共通規格であるVISA (Virtual Instrument Software Architecture)を，Pythonから利用できるようにしたものです．
...
**測定終了後**：出力の切り忘れによる事故防止のため，inst.close()を行う前に以下のコマンドを送信してください．
```
SOV 0; H
```
...
#### ロックインアンプ(LI5650)
ロックインアンプ (LI5650) を使用する際に必要なコマンドを以下に示します．

**アドレス**：今回用意した装置(LI5650)には以下のアドレスが割り当てられています．
```
GPIB0::10::INSTR
```
**初期化**
...
ここで返ってくるpeakPower,peakFreqは指定した周波数の範囲の中で単にパワーが最大となっている点の値です．ピークのパワーと中心周波数をより正確に求めるためには，これらの値を初期値として，ガウシアン(ローレンチアン)でフィッティングを行う必要があります．
...
**データの保存・読み出し**
様々な方法がありますが、一例として`numpy`モジュールの`savetxt,loadtxt`を用いる方法を紹介します。
...
np.savetxt(savepath, data_list, delimiter = ',')
```
`savepath`のパスで指定した場所に、`data_list`のデータをカンマ区切りで保存することができます。

```python
import numpy as np

loadpath = 'data.csv' 
load_list = np.loadtxt(loadpath, delimiter = ',')
```
`loadpath`のパスで指定した場所から、カンマ区切りのデータを`load_list`に読み出すことができます。
